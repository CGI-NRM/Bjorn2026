#' Import all excel files in a given directory as a single data frame
#'
...
ExcelToDF <- function(dir = ".", sheet = 1, pattern = ".xlsx") {
  ...
  if (length(filenames) == 0) {
      ...
      dfList <- lapply(filenames,
                        function(x) read_excel(path  = x, sheet = sheet))
      dfListTrim <- lapply(dfList, `[`, 1:4)
      if (length(unique(lapply(dfListTrim, function(x) names(x)))) != 1) {
        ...
        dfAll <- Reduce(function(x, y) merge(x, y, all = TRUE), dfListTrim)
        return(dfAll)
      }
  }
}
